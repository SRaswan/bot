...
class SampleControllerAsync(Node):
    ...
    def pupper(self):
        ...
        if self.phase == 1:
            print("Moving")
            if self.idx < len(self.stck):
                self.send_move_request(self.stck[idx])
                self.idx += 1
            else:
                self.phase = 2
                self.idx = 0
        ...
        if self.phase == 2:
            ...
            if response != -1:
                if response == self.stck[self.idx]:
                    self.idx += 1
                    print("Nice! Keep going")
                    if self.idx >= len(self.stck):
                        print("Correct! Moving to the next level.")
                        self.send_move_request(self.stck[-1])
                        self.phase = 0
                else:
